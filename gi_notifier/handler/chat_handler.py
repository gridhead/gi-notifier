...
async def handle_bot_added(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_member.new_chat_member.status in ["member", "administrator"]:
        ...
        await context.bot.send_message(chat_id=chat_id, text="Hello! I'm now part of this group.")
        config.logger.debug(f"Bot added to group: {title} (chat_id={chat_id})")

    elif chat_member.new_chat_member.status == "left":
        config.logger.debug(f"Bot removed from chat {chat_member.chat.title} ({chat_member.chat.id})")  #noqa: E501
